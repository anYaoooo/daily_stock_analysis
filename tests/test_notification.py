...
class TestNotificationServiceReportGeneration(unittest.TestCase):
    ...
    def test_generate_btc_single_stock_report_only_shows_strategy_and_technicals(
        ...
        self.assertIn("- 方向：**多单**", out)
        self.assertIn("- 入场：**65,000 USDT**", out)
        self.assertIn("- 止损：**63,200 USDT**", out)
        self.assertIn("- 止盈：**68,000 USDT**", out)
        self.assertIn("- 触发条件：放量突破", out)
        self.assertNotIn("- 入场：**突破 65000", out)
    ...
    def test_generate_btc_hourly_report_uses_hourly_schedule_template(
        ...
        self.assertIn("- 方向：**多单**", out)
        self.assertIn("- 入场：**64,800 USDT**", out)
