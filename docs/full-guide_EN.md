...
Technical signal gates are calibrated independently by trading phase (pre-market, intraday, lunch break, closing auction, or post-market), realized volatility (low, normal, high, or extreme), trend regime (bull, bear, range, or transition), and long/short direction. `trend_analysis.threshold_profile` records the dimension multipliers, bias distance, and long/short entry and strong-signal gates. Missing phase or volatility data falls back to `unknown` and `normal` respectively without stacking additional indicators.
The earlier decision-stability post-processing only adjusts advisory wording and stability logic and does not change the configured LLM model/provider routing semantics (including LiteLLM, providers, or API model settings).
...
On top of the P3 lifecycle, BTC plans are stably grouped by symbol plus horizon: `metadata.plan_key` is `crypto:BTC:intraday` or `crypto:BTC:3d`. A later same-direction report updates the canonical row in place, preserving its original source identity, market phase, existing context, and any execution fields already set for entry, stop, target, or invalidation. Missing execution fields may still be filled one by one, while fields omitted by a later report are not cleared, and neutral or avoid observations cannot downgrade an executable action. An opposite-direction report does not immediately replace the canonical plan. It is stored as an archived `reversal_candidate` with a parent-plan reference; only a new upstream structured plan may become canonical after the old plan is invalid or expired. Closed-bar confirmation remains the responsibility of BTC plan generation and the `btc-execution-v1` contract; the DecisionSignal lifecycle layer does not read candles again. An executable daily report selects `long_plan` or `short_plan` from its explicit action; when the overall action is neutral and both conditional sides exist, extraction no longer chooses long merely because it is listed first. An hourly report belongs only to `intraday_plan` and does not borrow a daily plan when that block is absent, so the two horizons cannot overwrite one another merely because they share a dashboard. When legacy duplicate active rows exist, subsequent BTC writes retain the earliest row as canonical and archive the rest. This is an application-level convergence rule, not a cross-process uniqueness guarantee, and it does not migrate old production rows automatically.
...
When history is deleted, an active BTC canonical plan carrying a `crypto:` plan key is not deleted with its source report. Its `source_report_id` is detached, and the original history ID plus deletion time are recorded in `metadata.source_history_deleted`. Terminal candidates and ordinary analysis signals are still cleaned up by the existing rule.
...
New plans also persist a versioned `tradeability_audit`. The pre-guard direction, levels, and execution contract are replayed as a same-window shadow counterfactual when the guard blocks a plan. Shadow outcomes appear only under `diagnostics.guard_forward_comparison.blocked_counterfactual`; they never affect primary win rate, equity, dynamic sizing, or risk metrics. Actual released-plan results appear under `released_actual`, while historical plans without audit metadata are counted in `legacy_unclassified_count` rather than classified from timestamps.
...
| Crypto | Common BTC spot symbols | `BTC`, `BTCUSDT`, `BTC-USD`, `BTC/USD` |
...
The primary shadow forecast defaults to a cost-aware 4-hour three-class target. A future return above round-trip cost is labeled `up`, one below negative cost is labeled `down`, and the middle band is `no_signal`. Each outer fold purges the end of its training window by the forecast horizon, then uses only an inner training tail to compare standardized Logistic, histogram gradient boosting, and optional LightGBM by multiclass Brier score. It also evaluates an equal-weight probability ensemble of available candidates; each candidate and the ensemble independently select probability shrinkage toward historical class rates, and the lowest inner Brier score wins. `primary_forecast` reports all three probabilities, conditional directional confidence, the selected model, available/missing candidates, and its calibration weight. It explicitly returns `no_signal` when directional confidence is below the configured threshold or neutral probability is too high. If LightGBM is missing, the diagnostic records `optional_dependency_missing` and falls back to the remaining candidates without failing BTC analysis.
...
After a selloff-rebound report produces an executable `selloff_rebound_trial_ready` trial signal, the volatility monitor keeps reading the persisted plan and follows the same frozen confirmation and invalidation levels. The first move back through the confirmation level emits a “possible false break” warning and blocks adding size without treating a single wick as hard invalidation. Persistence for `max(2, BTC_VOLATILITY_MONITOR_CONFIRMATION_SAMPLES)` polls upgrades the state to a short-window confirmed break and advises exiting or reducing the trial. A later reclaim emits a false-break-repaired alert. Touching the frozen invalidation level bypasses sample confirmation, marks the signal invalidated immediately, and advises exit. This state machine tracks a system-generated trial signal; it cannot prove that the user actually filled a position and never places or adds orders automatically.
