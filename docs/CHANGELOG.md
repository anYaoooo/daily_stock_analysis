...
- [修复] BTC 回测连续亏损熔断改按当前连续亏损计算，避免历史最大连败永久阻断新计划；已错过或失效的冻结计划会关闭旧的买卖信号，实时告警在通用行情短暂失败时改用 BTC 专用 REST 兜底。
...
- [文档] 增加 BTC 分析系统改造后复评报告：四层指标测量结果、上一份诊断建议落实情况与更新后的改进优先级。
- [改进] BTC 回测新增四维计划质量评分、动态权益仓位、费用/滑点敏感性矩阵及组合风险护栏诊断；动态 sizing 默认按上一笔完成权益计算，风险护栏仅审计提示，不改变历史成交或自动下单。
- [改进] BTC 回测拆分原始方向命中率（MFE/MAE）、信号质量率、执行成交率与成交后净结果命中率，统一生命周期状态；低量/价格结构冲突会确定性转为等待，缺失衍生品、订单流、EWMA 或宏观数据时明确降级并收紧仓位上限。
- [改进] BTC 逆日线小时计划单独限仓至原计划 50% 以内，并将入场等待窗口收紧至最多 6 根小时线；限仓原因和有效期通过 API/Web 回测记录透传。
- [改进] BTC 计划生成前置展示往返成本与目标缓冲，新增基于历史 MFE P70 的止盈距离诊断标注，并提供 OKX 1h 确定性方向投票离线基线评估脚本。
- [文档] 增加 BTC 分析系统底层逻辑、回测成交与亏损复盘及改进建议报告。
- [修复] BTC 回测页亏损复盘将内部指标枚举转换为中文含义，并以“命中数/亏损总数”展示共同特征，避免把共现频率误读为致亏原因。
- [修复] BTC 核心小时线交易上下文固定读取最近 7 天完整缓存，并与可选影子预测的长历史缓存解耦；长历史窗口不完整或预测依赖不可用时不再把已有小时线误报为“数据缺失”。
- [修复] BTC 计划校验新增确认价风险收益复核，报告风险收益文字统一按入场、止损、止盈确定性重算；避免理想入场看似合格、实际等确认后已不具备最低盈亏比的计划仍显示“校验通过”。
...
- [修复] BTC 日内计划在闭合日线与小时线已同向反对其方向时自动降为等待，避免多周期证据明确冲突时仍输出可执行交易方向；回测页同时区分实际成交与用于收益统计的独立成交，避免成交率口径混淆。
- [修复] BTC 小时线分析按自然日读取与波动监控一致的 OKX 永续合约缓存，避免本地已有行情却因现货身份或只取 7 根小时线而降级为“小时线数据缺失”。
- [新功能] BTC 分析、历史行情 API 与计划回测新增 SQLite 本地优先的闭合 K 线缓存；缓存按交易所、现货/永续、价格口径和周期隔离，覆盖缺口时才调用现有交易所数据源，并记录同步哈希以提高回测可复现性。
- [改进] BTC 波动监控将快速确认收敛为“真实越过确认价后减少确认次数”，新增脉冲阶段、过度延伸和衰竭识别；过度追价或脉冲衰竭不再被写成可立即执行的小时线计划。
- [修复] BTC 波动监控收敛高频采样下的噪声触发：多级窗口模式下速度触发地板抬升为 max(`VELOCITY_MIN_PCT`, 0.5×最快窗口阈值)，速度触发不再享受单次确认快进，反向打断冷却仅限真实窗口突破信号，避免短时间内反复推送多空相反指导。
- [改进] BTC 通知与历史报告新增“双向计划概览”：渲染日线多/空与小时线日内计划的方向、入场、止损、目标、触发与执行校验状态（✅/⚠️/⏸），未过校验计划附注说明，校验结果不再只存在于原始 JSON。
...
- [改进] BTC 报告将交易计划拆为试仓、确认加仓和失效退出三段，顶层可回测入场契约统一对应试仓层，避免确认点过晚而报告缺少当前可执行动作。
- [改进] BTC 回测拆分信号触发、委托与实际成交状态，并统计拒单后的错失有利与不利行情。
- [改进] BTC 活动交易计划在过期或失效前保持冻结，新增固定价位回踩与突破执行模板，避免趋势行情持续抬高触发价。
- [修复] Agent 分析路径在 BTC 计划落库前未执行风险收益与执行契约校验的问题。
...
- [文档] 新增 BTC 交易助手路线图，明确 BTC-only 收敛、结构化交易计划、一键回测、回测体验、风险控制和真实交易灰度推进顺序。
- [改进] 普通回测新增 `analysis_mode` 周期维度，日线分析与小时线分析分开落库、查询和汇总；BTC 小时线分析使用小时 K 线评估，普通股票小时线在缺少小时数据源时标记为数据不足，避免误用日线回测。
- [改进] Web 回测页新增日线/小时线周期筛选，结果列表和 BTC 回测表现可按周期查看。
- [改进] BTC 小时线定时分析改为每小时 05 分执行，以便获取上一根完整小时 K 线。
- [新功能] 新增 BTC 剧烈波动触发分析后台任务，schedule 模式可按实时价格短窗口波动自动触发小时线分析并输出进场机会，同时通过冷却和互斥避免重复报告。
- [改进] BTC 剧烈波动触发分析新增短窗口触发上下文与连续采样确认，报告会标明波动触发原因和未收线风险，并降低单次脏价格误触发概率。
- [改进] BTC 小时线分析新增急跌/扫低事件识别，并在 Prompt 中强制输出多单确认价、失效价和空单跌破价，避免暴跌后只给泛泛观望而缺少可执行进场条件。
- [修复] Web 回测页手动运行默认评估当前候选记录，并在没有新候选时显示明确提示，避免按钮表现为无响应。
- [改进] BTC 推送模板按日线与小时线定时任务拆分：日线消息标注 08:00 后更新并保留多空策略，小时线消息标注每小时 05 分更新并聚焦日内计划。
- [改进] BTC 定时分析拆分为北京时间 08:00 日线主分析与每小时小时线日内分析，且小时线不再被日线方向强制否决。
- [改进] BTC 分析 Prompt 改为加密货币专用语境，移除财报分红、换手率、市盈率、筹码分布等股票专属字段，并补充 ATR 波动率、真假突破识别、冲突判定和双向交易语义消歧。
...
- [改进] 定时 BTC 分析改为北京时间 08:00 执行日线主分析、每小时 05 分执行小时线日内分析，并允许小时线独立给出逆日线短线机会。
...
- [改进] 长报告通知按总长度均衡分页，避免企业微信等渠道第二条消息只剩少量尾部内容。
