...
> BTC-only 边界：股票市场复盘、A/H/美股数据源、基本面、股票索引、股票智能导入和旧 DecisionSignals Web UI 已退出默认产品与 CI 契约。遗留模块的隔离范围、审计命令和删除顺序见 [Legacy 股票模块隔离边界](legacy-stock-isolation.md)。本文档中仍描述这些能力的历史章节仅供迁移审计，不代表当前 Web 入口继续提供。
...
- 消息可保留 7 天
- 单条消息最多 1024 字符；长报告会按段落或换行分段并在标题标明页码，避免正文截断。
...
技术信号的评分门槛按交易阶段（盘前、盘中、午间、收盘竞价、盘后）、实现波动状态（低、正常、高、极端）、趋势阶段（多头、空头、区间、过渡）和多空方向分别校准。`trend_analysis.threshold_profile` 会记录各维度乘数、乖离阈值以及多空进场/强信号门槛；缺少阶段或波动数据时分别回退到 `unknown` 和 `normal`，不额外叠加指标。
此前的决策稳定性处理会影响可操作决策的运行时落盘与提示词约束链路，但不变更 LLM 模型、LiteLLM 路由、Provider/Key 及其兼容边界，不影响配置保存/清理语义。
...
BTC 计划在 P3 生命周期规则之上按“标的 + 周期”稳定归并：`metadata.plan_key` 使用 `crypto:BTC:intraday` 或 `crypto:BTC:3d`，同一周期的后续同向报告原地更新观察信息，保留首次主计划的来源身份、市场阶段、已有上下文和入场、止损、目标等执行字段；缺失的执行字段可以由后续报告逐项补齐，后续报告未提供的字段不会清空，观望或回避观察也不会把可执行动作降级。反方向报告不会立即替换主计划，而是作为 `archived` 的 `reversal_candidate` 保存，并记录父计划；原计划失效/过期后，新的上游结构化计划才可成为主计划。收盘确认由 BTC 计划生成与 `btc-execution-v1` 执行契约负责，DecisionSignal 生命周期层不重复读取 K 线判定。日线可执行报告按明确动作选择 `long_plan/short_plan`；当总动作为观望且同时存在多空条件计划时，不再按字段顺序强行选多。小时线报告只归属 `intraday_plan`，缺失该区块时也不会借用日线计划；两个周期不会因同一份 dashboard 互相覆盖。已有历史重复 active 记录会在后续 BTC 报告写入时保留最早记录为 canonical，其余归档；该逻辑不提供跨进程并发唯一性保证，也不会自动迁移生产库中的旧数据。
...
删除历史时，仍为 active 且带 `crypto:` plan key 的 BTC 主计划不会被连带删除；系统会解除其 `source_report_id`，并把原历史 ID 与删除时间写入 `metadata.source_history_deleted`。终态候选和普通分析信号仍按原规则清理。
...
新计划同时保存版本化 `tradeability_audit`。护栏拦截前的方向、点位和执行契约用于同窗口影子反事实回测；影子结果只进入 `diagnostics.guard_forward_comparison.blocked_counterfactual`，不参与正式胜率、权益、动态仓位或风险统计。放行计划的实际结果位于 `released_actual`，缺少审计元数据的历史计划计入 `legacy_unclassified_count`，系统不会按生成日期猜测护栏版本。

### BTC 亏损复盘

回测页会调用 `GET /api/v1/backtest/crypto/loss-review`，只读取当前回测引擎的已成交净亏损交易。复盘将亏损归入费用吞没毛利、止损/强平退出、方向判断失配或持有期内未达到目标，并返回实际成交、退出、费用、资金费和指标快照等证据，以及针对入场确认、目标位、持有期或成本门槛的改进建议。指标共同特征仅表示亏损样本中的出现频率，不是因果证明；没有与交易时点绑定的宏观或新闻证据时，接口会明确标记为不能归因外部因素。
...
| 加密货币 | BTC 常见现货代码 | `BTC`、`BTCUSDT`、`BTC-USD`、`BTC/USD` |
