...
const categoryDescriptionMap: Record<UiLanguage, Partial<Record<SystemConfigCategory, string>>> = {
  zh: {
  base: '管理 BTC 标的与基础运行参数。',
  data_source: '管理 BTC 行情、新闻与搜索数据源。',
  ...
  en: {
    base: 'Manage the BTC symbol and base runtime parameters.',
    data_source: 'Manage BTC market, news, and search data sources.',
...
const fieldTitleMap: Record<string, string> = {
  ...
  BTC_VOLATILITY_MONITOR_USE_WEBSOCKET: 'BTC 机会监控 WebSocket',
  BTC_VOLATILITY_MONITOR_WS_STALE_SECONDS: 'BTC WebSocket 缓存有效期（秒）',
...
const fieldDescriptionMap: Record<string, string> = {
  STOCK_LIST: 'BTC-only 模式固定使用 BTC；BTCUSDT、BTC-USD、BTC/USD 等别名会统一规范为 BTC。',
  ...
  BTC_VOLATILITY_MONITOR_USE_WEBSOCKET: '启用后，BTC 交易机会监控优先读取 Binance ticker WebSocket 缓存；缓存不可用或过期时回退 REST 行情。',
  BTC_VOLATILITY_MONITOR_WS_STALE_SECONDS: 'WebSocket 行情缓存的最长可接受年龄，超过后监控会回退 REST 行情。',
