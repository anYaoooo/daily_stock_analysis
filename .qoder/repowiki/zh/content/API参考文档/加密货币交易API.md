...
## 更新摘要
**所做更改**
- 新增CryptoOhlcvBar模型支持现货和永续合约工具的执行价格、标记价格和资金费率
- 增强管道集成，使用CryptoMarketDataService替代直接调用CryptoFetcher
...
**更新** 新增了支持现货和永续合约的增强型OHLCV数据模型，以及改进的数据服务层架构。
...
**更新** 现在支持区分执行价格和标记价格，特别适用于永续合约交易场景。
...
API->>Service : "调用业务方法"
Service->>MarketSvc : "获取增强型OHLCV数据"
...
**更新** 现在考虑永续合约的资金费率和标记价格溢价，提供更准确的交易信号。
...
**更新** 现在考虑永续合约的资金费率和标记价格，提供更精确的风险评估。
...
**更新** 现在支持区分执行价格和标记价格进行更精确的回测，特别适用于永续合约策略。
...
**更新** 新的增强型数据模型和市场数据服务提供了更强大的加密货币交易支持，特别是对于永续合约交易的执行价格与标记价格分离处理，以及资金费率的考虑。建议在生产环境启用缓存、监控与告警，持续优化性能与稳定性。

## 附录
- **集成指南**：前端使用TS客户端封装HTTP调用，遵循类型定义进行开发。
- **数据处理示例**：参考历史数据拉取与指标计算流程，结合缓存与降级策略。
- **最佳实践**：合理设置超时与重试、使用幂等键、记录关键决策日志。
- **更新** 建议使用CryptoMarketDataService而非直接调用CryptoFetcher，以获得更好的缓存管理和数据一致性保证。
