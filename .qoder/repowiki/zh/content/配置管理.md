...
## 更新摘要
**变更内容**   
- 新增BTC影子预测功能的完整配置支持，包括启用开关、回溯天数、最小训练bar数等环境变量配置
- 增强BTC波动率监控的入场追涨限制和衰竭回撤检测参数
- 完善BTC相关配置的验证规则和默认值设置
- 扩展配置注册表以支持新的BTC影子预测功能
...
**最新更新**：系统已集成完整的BTC影子预测和BTC波动率监控配置管理系统，包含超过300行的配置定义，支持实时波动检测、自适应阈值、速度触发、影子预测等高级功能。**新增BTC影子预测功能和增强的BTC波动率监控参数**，有效提升了加密货币市场分析和预测能力。
...
- **基础控制参数**
  ...
  - `BTC_SHADOW_FORECAST_LOOKBACK_DAYS`：回溯天数（默认60天，最小15天）
  - `BTC_SHADOW_FORECAST_MIN_TRAIN_BARS`：最小训练bar数（默认336，最小24）
  - `BTC_SHADOW_FORECAST_FOLDS`：交叉验证折数（默认5，最小1）
  - `BTC_SHADOW_FORECAST_VALIDATION_BARS`：验证bar数（默认24，最小1）
...
章节来源
- [src/config.py:742-746](file://src/config.py#L742-L746)
- [src/config.py:1620-1647](file://src/config.py#L1620-L1647)
- [src/services/btc_shadow_forecast_service.py:14-46](file://src/services/btc_shadow_forecast_service.py#L14-L46)
...
章节来源
- [src/config.py:747-775](file://src/config.py#L747-L775)
...
章节来源
- [src/config.py:742-775](file://src/config.py#L742-L775)
...
**最新进展**：系统已成功集成完整的BTC影子预测和BTC波动率监控配置管理系统，包含超过300行的配置定义，支持自适应阈值、速度触发、快速确认、影子预测等高级功能。**新增的BTC影子预测功能和增强的BTC波动率监控参数**进一步提升了系统的智能化水平，能够有效进行BTC小时级回报预测，避免垂直行情后的追价建议，并及时处理衰竭信号。这一增强使得系统能够实时监控BTC价格波动，并在检测到异常波动时自动触发分析流程，为加密货币交易决策提供及时的数据支持。
...
- **数据范围**：`BTC_SHADOW_FORECAST_LOOKBACK_DAYS` - 指定回溯分析的天数范围
- **训练要求**：`BTC_SHADOW_FORECAST_MIN_TRAIN_BARS` - 设置最小训练数据量要求
- **验证策略**：`BTC_SHADOW_FORECAST_FOLDS` 和 `BTC_SHADOW_FORECAST_VALIDATION_BARS` - 控制交叉验证的参数
...
- [src/config.py:742-746](file://src/config.py#L742-L746)
- [src/config.py:1620-1647](file://src/config.py#L1620-L1647)
- [tests/test_btc_shadow_forecast_service.py:41-104](file://tests/test_btc_shadow_forecast_service.py#L41-L104)
