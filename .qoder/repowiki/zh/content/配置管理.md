...
## 更新摘要
**变更内容**   
- 新增BTC波动率监控的入场追涨限制参数 `BTC_VOLATILITY_MONITOR_MAX_ENTRY_OVERSHOOT_PCT`（默认0.3%）
- 新增BTC波动率监控的衰竭回撤参数 `BTC_VOLATILITY_MONITOR_EXHAUSTION_RETRACE_PCT`（默认0.25%）
- 增强脉冲检测机制，防止垂直大阳/大阴后的追价建议
- 完善衰竭信号处理，在入场前出现大幅回撤时取消机会并发送衰竭警报
...
**最新更新**：系统已集成完整的BTC波动率监控配置管理系统，包含超过299行的配置定义，支持实时波动检测、自适应阈值、速度触发等高级功能。**新增入场追涨限制和衰竭回撤检测功能**，有效避免垂直行情后的追价建议和衰竭信号的及时处理。
...
- 全局配置入口（config.py）
  ...
  - **新增**：集成BTC波动率监控相关配置解析，包括新的入场追涨限制和衰竭回撤参数
...
- 关键设计
  ...
  - 懒加载减少启动开销
  - 校验失败时抛出明确错误，便于定位
  - **新增**：支持BTC波动率监控配置的高效处理，包括新的入场追涨限制和衰竭回撤参数
- 复杂度
  - 合并时间复杂度与配置项数量线性相关
  - 缓存命中后读取为常数时间
...
- **新增配置类别**：
  ...
  - **新增**：`BTC_VOLATILITY_MONITOR_MAX_ENTRY_OVERSHOOT_PCT`：入场追涨限制（0.3%）
  - **新增**：`BTC_VOLATILITY_MONITOR_EXHAUSTION_RETRACE_PCT`：衰竭回撤阈值（0.25%）
...
- [src/config.py:1750-1762](file://src/config.py#L1750-L1762)

### 系统配置服务（system_config_service.py）
- 职责
  - 封装配置的增删改查、校验、持久化与事件广播
  - 保证事务性与一致性
  - **新增**：支持BTC波动率监控配置的热更新，包括新的入场追涨限制和衰竭回撤参数
- 关键设计
  - 读写分离：读路径走缓存，写路径落盘并触发热更新
  - 审计日志：记录每次变更的来源与差异
  - **新增**：BTC监控配置变更时自动重启监控器，确保新参数立即生效
...
- 职责
  ...
  - 输入校验与错误响应标准化
  - **新增**：支持BTC波动率监控配置的Web界面管理，包括新的入场追涨限制和衰竭回撤参数
- 关键设计
  - 使用Pydantic模型进行请求/响应校验
  - 权限控制与操作审计
  - **新增**：BTC监控配置的专用验证规则，确保新参数的合理性
...
- 职责
  - 封装API调用、状态管理与实时更新
  - 提供类型安全的配置访问
  - **新增**：BTC波动率监控配置的Web界面支持，包括新的入场追涨限制和衰竭回撤参数
- 关键设计
  - 使用Hook封装订阅与刷新逻辑
  - 本地缓存与乐观更新提升用户体验
  - **新增**：BTC监控配置的分组展示和验证反馈，包括新参数的说明和示例
...
- 职责
  - 在启动前检查必需的环境变量与配置完整性
  - 在容器中执行必要的初始化步骤
  - **新增**：BTC波动率监控相关环境变量的验证，包括新的入场追涨限制和衰竭回撤参数
- 关键设计
  - 失败即退出，避免运行期错误
  - 输出清晰的诊断信息
  - **新增**：BTC监控配置的有效性检查，确保新参数的合理性和安全性
...
- 组件耦合
  ...
  - **新增**：BTC波动率监控依赖配置管理器提供的配置，包括新的入场追涨限制和衰竭回撤参数
- 外部依赖
  - 环境变量与配置文件作为配置来源
  - 数据库或文件系统用于持久化
  - 消息通道（可选）用于配置变更广播
...
- 启动优化
  - 延迟加载配置，仅在首次访问时解析
  - 批量合并与一次性校验，减少重复计算
  - **新增**：BTC监控配置的懒加载，避免启动时不必要的计算，包括新的入场追涨限制和衰竭回撤参数
- 运行时优化
  - 读路径使用内存缓存，避免频繁I/O
  - 写路径异步持久化，降低阻塞
  - **新增**：BTC监控配置变更时的增量更新，确保新参数立即生效
- 网络与前端
  - 前端缓存与增量更新，减少重复请求
  - 使用WebSocket或SSE推送配置变更（可选）
  - **新增**：BTC监控配置的前端实时预览，包括新参数的可视化展示
...
- 常见问题
  ...
  - **新增**：BTC监控配置参数超出有效范围
  - **新增**：加密货币数据源连接失败
  - **新增**：入场追涨限制和衰竭回撤参数配置不当导致的误报或漏报
- 排查步骤
  - 使用环境变量检查脚本验证必需项
  - 查看启动日志中的配置加载与校验错误
  - 通过API返回的错误码与消息定位问题
  - 检查持久化存储的权限与可用性
  - **新增**：检查BTC监控配置的有效性和合理性，特别是新参数的取值范围
  - **新增**：验证加密货币数据源的连通性
  - **新增**：调试入场追涨限制和衰竭回撤的检测逻辑
- 恢复策略
  - 回滚到上一个已知正确的配置版本
  - 临时关闭非关键配置项以恢复服务
  - 启用更详细的调试日志以便定位
  - **新增**：重置BTC监控配置到默认值，包括新参数的默认配置
...
**最新进展**：系统已成功集成完整的BTC波动率监控配置管理系统，包含299行以上的配置定义，支持自适应阈值、速度触发、快速确认等高级功能。**新增的入场追涨限制和衰竭回撤检测功能**进一步提升了系统的智能化水平，能够有效避免垂直行情后的追价建议，并及时处理衰竭信号。这一增强使得系统能够实时监控BTC价格波动，并在检测到异常波动时自动触发分析流程，为加密货币交易决策提供及时的数据支持。
...
- 默认值策略
  ...
  - 敏感字段不提供默认值，强制显式配置
  - 环境差异化通过环境变量覆盖默认值
  - **新增**：BTC监控配置提供保守的默认值以确保稳定性，包括入场追涨限制0.3%和衰竭回撤0.25%
...
- 版本管理
  - 每次变更生成版本号与差异摘要
  - 支持回滚到历史版本
  - **新增**：BTC监控配置版本的独立管理，包括新参数的版本控制
- 多环境策略
  - 通过环境变量区分环境（开发、测试、生产）
  - 配置文件仅包含非敏感与环境无关项
  - 敏感信息通过密钥管理服务注入
  - **新增**：不同环境下BTC监控参数的差异化配置，包括入场追涨限制和衰竭回撤参数的环境适配
...
- 迁移工具
  ...
  - 自动补全缺失字段与默认值
  - 生成迁移报告与回滚方案
  - **新增**：BTC监控配置的迁移支持，包括新参数的自动迁移和兼容性处理
- 升级指南
  - 预检查：验证环境与依赖
  - 灰度发布：先小流量验证
  - 监控与回滚：实时监控指标，快速回滚
  - **新增**：BTC监控功能的渐进式启用，包括新参数的分阶段部署
...
- 容错：重试与降级策略，避免队列阻塞
- **新增**：BTC监控任务队列的配置同步机制，包括新参数的同步处理
...
5. **入场追涨限制**：防止价格越过确认价过远时的追涨行为
6. **衰竭回撤检测**：识别脉冲行情在入场前的衰竭信号

#### **新增功能详解**
- **入场追涨限制机制**
  - 参数：`BTC_VOLATILITY_MONITOR_MAX_ENTRY_OVERSHOOT_PCT`（默认0.3%）
  - 作用：避免垂直大阳/大阴后才给出追价建议
  - 效果：当价格越过确认价超过此阈值时，仍可生成小时线分析，但日内计划会强制降为等待
  - 适用场景：防止在市场极端波动时的盲目追涨

- **衰竭回撤检测机制**
  - 参数：`BTC_VOLATILITY_MONITOR_EXHAUSTION_RETRACE_PCT`（默认0.25%）
  - 作用：检测脉冲行情在入场前的衰竭信号
  - 效果：若脉冲已越过确认价、随后在入场前回撤/反抽达到此阈值，机会会被取消并发送衰竭警报
  - 适用场景：识别虚假突破和假突破信号
...
- [src/core/config_registry.py:3800-4322](file://src/core/config_registry.py#L3800-L4322)
- [docs/full-guide.md:1693-1726](file://docs/full-guide.md#L1693-L1726)