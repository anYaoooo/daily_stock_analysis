...
- BTC影子预测：基于小时线数据构建轻量模型，输出下一小时预期收益率与涨跌概率，仅供观察与离线校准，不直接参与交易决策。
...
## 核心组件
- BTC影子预测服务：对小时K线进行特征工程与滚动窗口验证，输出预期收益与涨跌概率，明确标记不参与交易决策。
...
- 模型训练：Ridge回归估计期望收益；Logistic回归估计涨跌概率；每折独立缩放，避免数据泄露。
- 评估：扩展式滚动窗口（expanding walk-forward），输出MAE、方向准确率、Brier Score等。
...
WalkForward --> FitModel["Ridge回归 + Logistic回归"]
FitModel --> Predict["预测期望收益与涨跌概率"]
Predict --> Output["输出影子预测结果<br/>含数据质量与说明"]
...
## 结论
BTC影子预测服务以轻量、稳健的方式提供下一小时预期收益与涨跌概率，明确限定为观察与校准用途，不干扰交易决策。结合波动触发监控，系统能够在市场异动时及时预警并触发深入分析，形成“预警—确认—计划”的闭环。配合FastAPI与Web界面，用户可便捷地查看行情、分析报告与交易状态，实现从数据到洞察的一体化体验。
